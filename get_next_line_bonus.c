...
int		get_next_line(int fd, char **line)
{
	...
	{
		buf_line[rd] = '\0';
		if (!(ostatok[fd] = ft_processing(&ostatok[fd], buf_line)))
			return (-1);
		if (ft_strchr(ostatok[fd], '\n'))
			return (ft_write_to_line(&ostatok[fd], line, rd));
		else if (ft_strchr(ostatok[fd], '\0') && rd == 0)
		{
			if (ft_write_to_line(&ostatok[fd], line, rd) == -1)
				return (-1);
			ostatok[fd] = NULL;
			break ;
		}
